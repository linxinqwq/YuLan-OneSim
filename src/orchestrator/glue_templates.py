...
def is_rumor_propagation_requirement(text: str) -> bool:
    keys = ("谣言", "信谣", "转发", "恐慌", "辟谣", "传播", "社交网络", "级联", "扩散")
    return sum(1 for k in keys if k in text) >= 2


def rumor_propagation_glue_code(ids: List[str], req_short: str) -> str:
    ids_lit = json.dumps(ids, ensure_ascii=True)
    return f'''"""
谣言传播与恐慌扩散 — 组合仿真（编排器生成）
匹配 YuLan 场景 ID: {ids_lit}
需求摘要: {req_short}
"""
from __future__ import annotations
...
def generic_glue_code(ids: List[str], req_short: str) -> str:
    """非谣言类需求的简易占位。"""
    return rumor_propagation_glue_code(
        ids if ids and ids[0] != "_stub_" else [
            "emotional_contagion_model",
            "information_cascade_and_silence",
            "diffusion_of_innovations",
        ],
